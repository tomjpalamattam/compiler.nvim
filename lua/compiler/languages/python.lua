...
function M.action(selected_option)
  ...
  local function dump_block_to_temp()
    ...
    vim.fn.writefile(vim.api.nvim_buf_get_lines(bufnr, start-1, finish, false), temp_py)
    return temp_py
  end
    ...
    if solution_file then
      ...
      for entry, vars in pairs(config) do
        if entry ~= "executables" then
          ...
          local args = vars.arguments or "--warn-implicit-exceptions --warn-unusual-code"
          table.insert(tasks, {
      ...
      for _, ep in ipairs(entry_points) do
        ...
        local out_dir = utils.os_path(path:match("^(.-[/\\])[^/\\]*$") .. "bin")
    ...
    local task = overseer.new_task({
      ...
      strategy = { "orchestrator", tasks = {{
        ...
        cmd        = "rm -f \"" .. output .. "\" || true" ..
                     ...
                     " --workpath \"" .. cache_dir .. "\"" ..
                     " --specpath \"" .. cache_dir .. "\"" ..
                     " --onefile --distpath \"" .. output_dir .. "\" " .. args ..
                     " && \"" .. output .. "\"" ..
                     " && echo \"" .. entry_point .. "\"" ..
                     " && echo \"" .. final_message .. "\"",
        components = { "default_extended" },
